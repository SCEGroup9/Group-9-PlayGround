 ...
 if(isset($_POST['login'])){
    $username = $_POST['username'];
     if ($_SESSION['username'] == $username){
        echo "<script>alert('user $username already connected'); window.location = './User2log.php';</script>";
     }
     else{
        $psw = $_POST['psw'];
        ...
        if($numrows == 1){
           ...
           if ($today === $row["tDate"]){
            ...
            echo "<script>alert('Welcome $username you are now connected as user'); window.location = './cards.php';</script>";
       }
       else {
...
<head>
    ...
    <body> 
        ...
        <form id="loginMember" role="form" action="User2log.php" method="post">
