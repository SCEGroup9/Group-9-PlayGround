...
if (isset($_SESSION['username2'])){
    ...
    $sqlm = "SELECT * FROM `precords` WHERE `user` = '$user2' AND `gamename` = 'cards'"; //update loss to player 2
    ...
    if($numrow == 1){
        ...
        $a = $rows["losses"]+1;
        $b = $rows["countgames"]+1;
        $sqlm1 = "UPDATE `precords` SET `losses` = '$a', `countgames` = $b WHERE `user` = '$user2'";
        mysqli_query($db6, $sqlm1);
    }  
    else {
        $sqlm2 = "INSERT INTO precords(user, gamename, points, wins, losses, countgames) 
                    VALUES('$user2', 'cards', '0', '0', '1','1')"; //set new one
