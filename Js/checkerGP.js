...
turnID.innerHTML=user1+" turn now";
score.innerHTML=user2+"  VS  "+user1;
...
function checkWinner(){
    ...
    if(white==0){
        winner.innerHTML=user2+" is the winner"; frame();
        window.open("/Html/checkerwin2.php");
    }
    if(black==0){
        winner.innerHTML=user1+" is the winner"; frame();
        window.open("/Html/checkerwin1.php");
    }
}
function frame(){
...
function movepiece(){ 
    ...
    if(curCube.style.backgroundColor=='green'){ //console.log(ids);
        ...
        if(turn==1){ turnID.innerHTML=user2+" turn now"; turnID.style.color="red";}     //change to username later
        else {turnID.innerHTML=user1+" turn now"; turnID.style.color="white";}
